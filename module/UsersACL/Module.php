...
class Module
{
    ...
    function boforeDispatch(MvcEvent $event)
    {
        ...
        if (! ((strpos($name, 'reset-password') || in_array($name, $list))) && $session->offsetExists('userId')) {
            ...
            if (! $status) {
                die('Acesso Negado! Volte a pagina do seu navegador para continuar acessando.');
                //echo 'Você não tem acesso a este recurso. Volte a página de seu navegador para retornar ao Sistema!';
            }
        }
    }
